...
for jsx_path in sorted(glob.glob(os.path.join(base, "*", "build_psd.jsx"))):
    ...
    contain = 0; overlap = 0; small = 0
    for i, a in enumerate(boxes):
        aw = a["x2"] - a["x1"]; ah = a["y2"] - a["y1"]
        if aw < 14 or ah < 14: small += 1
        for j, b in enumerate(boxes):
            if i >= j: continue
            if (a["x1"] <= b["x1"] and a["y1"] <= b["y1"] and a["x2"] >= b["x2"] and a["y2"] >= b["y2"]): contain += 1
            elif (b["x1"] <= a["x1"] and b["y1"] <= a["y1"] and b["x2"] >= a["x2"] and b["y2"] >= a["y2"]): contain += 1
            ...
            if oa > 0:
                ...
                if oa / ma > 0.20: overlap += 1

    print(f"{name}: {len(groups)} rows, {len(boxes)} boxes, contain={contain}, overlap={overlap}, sub14px={small}")
